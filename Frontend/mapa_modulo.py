...
class MapaManager:
    ...
    def actualizar_campos_y_mapa(self, lat, lon):
        ...
        self.parent.coord_label.configure(text=f"Coordenadas seleccionadas:\nLat: {lat:.5f}\nLon: {lon:.5f}")
    ...
    def descargar_datos(self):
        ...
        try:
            ...
            CTkMessagebox(title="✅ Descarga exitosa",
                          message=f"Datos cargados correctamente.\nFilas: {len(df)}")

        except Exception as e:
            CTkMessagebox(title="❌ Error", message=f"No se pudo leer el CSV.\n{e}")
